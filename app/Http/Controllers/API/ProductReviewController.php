...
class ProductReviewController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($this->isEmpty($productreviews)) {
            ...
            $data['data'] = null;
            $data['maximumPages'] = 0;
            $data['msg'] = $this->getNotFoundMsg('ProductReviews');
            $data['code'] = 404;
            return response()->json($data, 404);
        } else {
            //Page Pagination Result List
            //Default return 10
            $paginateddata = $this->paginateResult($productreviews, $request->pageSize, $request->pageNumber);
            $data['status'] = 'success';
            $data['data'] = $paginateddata;
            $data['maximumPages'] = $this->getMaximumPaginationPage($productreviews->count(), $request->pageSize);
            $data['msg'] = $this->getRetrievedSuccessMsg('ProductReviews');
            $data['code'] = 200;
